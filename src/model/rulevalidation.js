...
module.exports = {
  ...
  TimeIsInRange: function (rule) {
    ...
    // prepare from time
    const from = new Date(new Date().toLocaleString())
    ...
    // prepare to time
    const to = new Date(new Date().toLocaleString())
  ...
  TodayIsInRuleRange: function (rule) {
    // TODO: check if rule is valid
    const now = new Date(new Date().toLocaleString())
    const concatnowstr = padStart(now.getMonth() + 1, 2, '0') + padStart(now.getDate(), 2, '0')
    let arr = rule.From.split(';')
    const from = arr[1] + arr[2]
    arr = rule.To.split(';')
    const to = arr[1] + arr[2]
